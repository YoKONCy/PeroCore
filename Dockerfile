FROM python:3.10-slim AS python-builder
 ...
 COPY --from=python-builder /tmp/wheels/*.whl /tmp/wheels/
 RUN pip install --no-cache-dir /tmp/wheels/*.whl && rm -rf /tmp/wheels
 
 COPY --from=python-builder /app/target/wasm32-unknown-unknown/release/nit_terminal_auditor.wasm \
     /app/backend/nit_core/tools/work/TerminalExecutor/auditor.wasm
 COPY --from=python-builder /app/target/release/CodeSearcher \
