...
运行方式:
    cd PeroCore/backend
    python -m benchmarks.aura_vision_benchmark
"""
...
class AuraVisionBenchmark:
    ...
    def setup(self) -> bool:
        ...
        if not self.model_path.exists():
            print(f"⚠️ 模型文件不存在，将使用无模型模式进行基础测试")
    ...
    def benchmark_vector_quality(self, num_samples: int = 50):
        ...
        for i in range(num_samples):
            pixels = self.generate_random_pixels(seed=i)
            result = self.manager.search_intent(pixels, top_k=1)
            # 注意: search_intent 返回的是 (id, similarity, description)
            # 我们无法直接获取向量，但可以通过相似度间接验证
            # 这里我们使用一个简化的测试方法
