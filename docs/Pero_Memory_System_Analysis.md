...
### 3.3 检索算法流程 (Retrieval Pipeline)

目前系统采用 **"后过滤" (Post-filtering)** 架构：

1.  **向量召回**: Python 调用 Rust 核心，根据语义相似度快速捞出前 N 个（例如前 20 个）最接近的记忆 ID。
2.  **元数据比对**: Python 拿到 ID 后，回查 SQLite 获取 `timestamp`、`tags` 等信息，根据过滤条件进行硬性过滤。

#### A. 架构局限性分析 (Bottlenecks)
这种“先召回，再过滤”的结构在记忆量级较小时（万级以内）非常高效，但在以下场景存在失效风险：
*   **检索精度流失**: 如果满足过滤条件的条目在向量距离上排在第 21 名开外，即便它是数据库中唯一符合条件的，也会被第一步的 Rust 引擎丢弃，导致最终结果为空。
*   **计算冗余**: 为了提高召回率，系统有时不得不大幅调大召回数量（如请求 Top 100），这增加了 Python 层的处理开销。

#### B. 优化路线图 (Roadmap)
*   **V2.5 (短期)**: **动态召回补偿**。根据过滤条件的严苛程度，自动在 Python 层调大召回倍率。
*   **V3.0 (中期)**: **位图掩码检索 (Bitmask Search)**。

##### 什么是位图掩码 (Bitmask)？
位图是一种极其紧凑的二进制数据结构。假设系统有 100 万条记忆，位图就是 100 万个 bit（仅约 125KB）。
1.  **预处理**: Python 侧通过 SQLite 快速查出符合过滤条件（如 `timestamp > yesterday`）的所有 ID。
2.  **生成位图**: 将符合条件的 ID 位置设为 1，其余为 0。
3.  **Rust 执行**: 将位图传给 Rust 核心。Rust 在 HNSW 图中跳转搜索时，每到一个节点先检查位图，只有 bit 为 1 的节点才计算向量距离。

##### 位图掩码的缺点与挑战
*   **跨语言传递开销**: 虽然位图很小，但频繁地在 Python 与 Rust 之间序列化和传递大数据块仍有性能损耗。
*   **稀疏性陷阱 (The Sparse Filter Problem)**: 
    *   **现象**: 如果过滤条件极其严苛（例如 100 万条记忆中只有 1 条符合），位图中 99.99% 都是 0。
    *   **后果**: HNSW 算法依赖于节点的“邻居”来寻找方向。如果当前节点的邻居在位图中全是 0，搜索算法会像在漆黑的迷宫里乱撞，导致检索速度退化到甚至不如全表扫描，或者直接找不到结果。
*   **内存一致性**: 向量索引 ID 与数据库自增 ID 必须保持绝对一致，否则位图映射会完全错位。

*   **V4.0 (长期)**: **原生混合索引**。将高频元数据（如日期、核心标签）直接编码进向量索引，实现端到端的“真·混合检索”。
