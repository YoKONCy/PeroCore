...
工作上下文由系统自动保存最近几轮读取文件、代码搜索或网页读取所获得的文本正文及必要来源信息，不是工具日志或执行记录。系统只采集明确的文本获取工具，不会保存 ReAct 过程叙述、终端输出、扩展日志、状态回执、目录与文件元数据、原始工具调用参数或最终回复；每轮记录会在后续 {{ owner_appellation }} 与你完成若干轮对话后独立过期。
