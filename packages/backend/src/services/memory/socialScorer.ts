 ...
 *
 * @module packages/backend/src/services/memory/socialScorer
 */
...
export class SocialScorerService {
  ...
  async processBatch(agentId: string): Promise<void> {
    ...
    try {
      // 渲染 MDP 模板
      const prompt = this.mdpEngine.render('tasks/memory/scorer/social_segment_summarizer', {
   ...
   *
   * 节点类型:
   * - event: 事件摘要 (零向量, BM25 索引)
   * - feature: 关键词 (零向量, AC 自动机索引)
   *
   * 边类型:
   * - memory_edge: Feature → Event (联想到具体事件)
   * - representation: Feature → Feature (单向联想)
   * - equality: Feature ↔ Feature (双向等价)
   */
