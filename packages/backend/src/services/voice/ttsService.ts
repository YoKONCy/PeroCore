...
export class TtsService {
  ...
  async synthesize(request: TtsRequest): Promise<TtsResult> {
    ...
    switch (provider) {
      ...
      case 'azure':
        throw new AppError('CONFIG_ERROR', { message: 'Azure TTS 尚未实现' })
      case 'local':
        throw new AppError('CONFIG_ERROR', { message: '本地 TTS 尚未实现' })
      default:
  ...
  private async synthesizeEdge(request: TtsRequest): Promise<TtsResult> {
    ...
    try {
      ...
      throw new AppError('CONFIG_ERROR', { message: 'msedge-tts 包未安装，请运行: pnpm add msedge-tts' })
    }
  ...
  private async synthesizeOpenAI(request: TtsRequest): Promise<TtsResult> {
    ...
    if (!apiKey) {
      throw new AppError('CONFIG_ERROR', { message: 'OpenAI TTS 需要 API Key，请在 VoiceTab 中配置' })
    }
    ...
    if (!response.ok) {
      ...
      throw new AppError('EXTERNAL_ERROR', { message: `TTS API 调用失败: ${response.status}` })
    }
