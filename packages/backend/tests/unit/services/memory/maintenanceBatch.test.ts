...
import { ConversationLogService } from '@perocore/backend/services/memory/conversationLog'
...
describe('ConversationLogService', () => {
  function createRepo(logs: Array<Record<string, unknown>> = []) {
    return {
      savePair: vi.fn(() =>
        Promise.resolve([
          { id: 1, role: 'user' },
      ...
      getRecent: vi.fn(() => Promise.resolve(logs)),
      query: vi.fn(() => Promise.resolve(logs)),
      findPendingPairs: vi.fn(() => Promise.resolve([{ pairId: 'p1' }])),
      updateMeta: vi.fn(() => Promise.resolve()),
      count: vi.fn(() => Promise.resolve(2)),
      deleteBySession: vi.fn(() => Promise.resolve()),
      updateContent: vi.fn(() => Promise.resolve(true)),
      deleteById: vi.fn(() => Promise.resolve(true)),
      findById: vi.fn((id: number) => Promise.resolve(logs.find((item) => item.id === id) ?? null)),
      deleteByPairId: vi.fn(() => Promise.resolve(2)),
    }
  }

  it('应当保存对话对、获取上下文窗口并代理分析状态操作', async () => {
    const repo = createRepo([
      { role: 'user', content: '你好' },
      { role: 'assistant', content: '在的' },
    ])
    const service = new ConversationLogService(repo as never)

    const saved = await service.savePair({
      agentId: 'pero',
      sessionId: 's1',
      source: 'desktop',
      user: '你好',
    ...
    const context = await service.getContextWindow('pero', 's1', 2)
    await service.markAnalyzed('p1', { sentiment: 'positive', importance: 5, memoryId: 9 })
    await service.markFailed('p2', '失败')

    expect(saved).toMatchObject({ userLogId: 1, assistantLogId: 2 })
    expect(saved.pairId).toMatch(/^\d+_[a-z0-9]+$/)
    expect(context).toEqual([
      { role: 'user', content: '你好' },
      { role: 'assistant', content: '在的' },
    ])
    expect(repo.updateMeta).toHaveBeenCalledWith('p1', {
      sentiment: 'positive',
      importance: 5,
      memoryId: 9,
      analysisStatus: 'done',
    })
    expect(repo.updateMeta).toHaveBeenCalledWith('p2', {
      analysisStatus: 'failed',
      lastError: '失败',
    })
  })

  it('应当支持编辑、删除、级联删除和清空 Agent 会话', async () => {
    const repo = createRepo([
      {
        id: 1,
        pairId: 'p1',
    ...
    await expect(service.updateMessage(1, '新内容')).resolves.toBe(true)
    await expect(service.deleteMessage(1)).resolves.toBe(true)
    await expect(service.deleteMessagePair(1)).resolves.toBe(2)
    await expect(service.deleteMessagePair(2)).resolves.toBe(1)
    await expect(service.deleteMessagePair(99)).resolves.toBe(0)
    await expect(service.deleteAllSessions('pero')).resolves.toBe(2)

    expect(repo.updateContent).toHaveBeenCalledWith(1, '新内容')
    expect(repo.deleteByPairId).toHaveBeenCalledWith('p1')
    expect(repo.deleteBySession).toHaveBeenCalledWith('s1', 'pero')
    expect(repo.deleteBySession).toHaveBeenCalledWith('s2', 'pero')
  })

  it('应当按会话生成摘要并分页排序', async () => {
    const repo = createRepo([
      {
        sessionId: 's1',
        agentId: 'pero',
    ...
    const summaries = await service.listSessionSummaries({ agentId: 'pero', page: 1, pageSize: 1 })
