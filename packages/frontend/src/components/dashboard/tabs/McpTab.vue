 ...
 * 数据通过 useMcpConfig composable 持久化到 configApi KV。
 */
  ...
  <div class="tab-mcp">
        ...
        <div class="mcp-server-main">
          ...
          <div class="mcp-server-info">
            <div class="mcp-server-name-row">
              ...
              <span :class="['mcp-server-status', statusMeta[srv.status]!.class]">{{
                statusMeta[srv.status]!.label
              }}</span>
    ...
    <PDialog v-model="isAddOpen" title="添加 MCP 服务" width="480px">
      <div class="mcp-add-form">
        <div class="mcp-field">
          <label class="mcp-label">名称</label
          ><PInput v-model="addForm.name" placeholder="服务名称" />
        </div>
        <div class="mcp-field">
          <label class="mcp-label">连接类型</label
        ...
        <div v-if="addForm.type === 'stdio'" class="mcp-field">
          ...
          ><PInput v-model="addForm.command" placeholder="npx @modelcontextprotocol/server-xxx" />
        </div>
        <div v-else class="mcp-field">
          <label class="mcp-label">WebSocket URL</label
          ><PInput v-model="addForm.url" placeholder="ws://localhost:8080" />
        </div>
      </div>
      <template #footer>
...
.mcp-list::-webkit-scrollbar {
  width: 4px;
}
.mcp-list::-webkit-scrollbar-thumb {
