...
*   **改进**：我用 Rust 重写了 HNSW 的核心算子，并针对 Intel/AMD 的 **SIMD (AVX2/AVX-512)** 指令集做了手动对齐。
*   **核心加速逻辑**：为什么能这么快？因为我意识到在“记忆召回”场景下，我们不需要遍历整个宇宙。在 KDN 算子中，我强制实施了 **Top-10,000 活跃限制**。每一轮扩散，我们只处理当前能量最高的前一万个节点。
*   **为什么是 10,000？**：因为对于 AI 桌面助手来说，长时记忆的精髓在于“精准”而非“全量”。10,000 个活跃节点已经足以覆盖一个极其庞大且复杂的对话上下文或知识背景。这种“有损但高效”的设计，让复杂度不再随数据总量线性增长，而是锁死在毫秒级。
