...
### 7.3 安全约束 (Security Constraints)

为了防止 AI 产生幻觉或受到恶意 Prompt 注入导致系统风险，NIT 2.0 实施了多项安全硬约束：

#### 7.3.1 输入长度截断 (Anti-ReDoS)
*   **物理截断**: 在 Rust 文本清洗层 (`TextSanitizer`)，所有传入字符串在正则处理前均会被物理截断至 **100,000 字符**。
*   **目的**: 防止精心构造的超长文本引发正则回溯攻击 (ReDoS)，确保 CPU 负载平稳。

#### 7.3.2 变量配额 (Resource Quotas)
*   **变量上限**: 单个 `<nit>` 块内定义的变量不得超过 **100 个**。
*   **字符串上限**: 存储在变量中的字符串值最大长度限制为 **100 KB**。

#### 7.3.3 文件系统隔离 (Path Traversal Defense)
*   **根路径锚定**: `file_ops.py` 下的所有操作（`read_file_content`, `list_directory` 等）均被强制锚定在 `PeroCore\backend\nit_core\pero_workspace` 目录下。
*   **逃逸检测**: 
    *   系统会自动解析所有输入路径为绝对路径。
    *   任何尝试通过 `..` 或绝对路径访问该目录之外的行为都将触发 `PermissionError` 并返回 "Access Denied"。
*   **工作模式强制**: 只有在开启 `Work Mode` 且经过身份校验后，才允许调用此类敏感文件接口。
